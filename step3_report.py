...
def build_predictions_html(pred_data, backtest_data=None):
    ...
    for p in top5:
        ...
        html += f"""<div class="stock-card" style="padding:10px;margin:6px 0">
            <div style="display:flex;justify-content:space-between;align-items:center">
                ...
                <div style="text-align:right"><span style="color:#ef4444;font-weight:bold;font-size:16px">{score:.1f}</span><br><span style="font-size:11px;color:#94a3b8">{p.get('信号','')}</span></div>
            </div>
    ...
    for p in bottom5:
        ...
        html += f"""<div class="stock-card" style="padding:10px;margin:6px 0">
            <div style="display:flex;justify-content:space-between;align-items:center">
                <div><strong>{p['名称']}</strong> <span style="color:#64748b;font-size:12px">{p['代码']}</span></div>
                <div style="text-align:right"><span style="color:#22c55e;font-weight:bold;font-size:16px">{score:.1f}</span><br><span style="font-size:11px;color:#94a3b8">{p.get('信号','')}</span></div>
            </div>
            <div style="margin-top:6px">{build_factor_bars_html(p)}</div>
        </div>"""
    html += "</div></div>"
